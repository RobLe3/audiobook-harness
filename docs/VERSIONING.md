...
Audiobook Harness **0.4.9** is the current product release. It adds
transactional phase-output validation, input-bound repair tickets, and separate
source-bound listener findings. Immutable take/render lineage, item-scoped
decisions, typed chapter isolation, codec-aware tail validation, phase-scoped
continuation, and strict hash-bound verification remain mandatory.
...
product versions. New reports identify the product as `0.4.9` and bind the
