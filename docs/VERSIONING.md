...
Audiobook Harness **0.7.0** is the current product release. It adds an opt-in,
project-scoped convergence worker that can start with Review Center, resume
machine-actionable work, and stop at a listener or evidence boundary. Review
requests remain read-only and promotion remains separate. Policy-owned acoustic
...
The supported maturity claim for 0.7.0 is controlled maintainer-operated local
...
product versions. New reports identify the product as `0.7.0` and bind the
