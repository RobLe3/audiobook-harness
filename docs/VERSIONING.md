...
Audiobook Harness **0.4.12** is the current product release. It adds an
evidence-fused post-generation diagnosis, strategy-specific repair plans,
optional advisory scorer ingestion, deterministic boundary measurements, and
listener-outcome learning without automatic approval authority. The existing
...
product versions. New reports identify the product as `0.4.12` and bind the
