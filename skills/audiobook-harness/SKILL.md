...
6. Run `generate`, then `verify`.
7. If any take fails ASR, timing, pronunciation, clipping, or duration checks, revise the lexicon/context strategy and regenerate only that take. Do not publish a failed take.
8. Run `release` only after verification passes.
